...
async function fetchApplicants() {
  ...
  try {
    ...
    if (data.success && data.data) {
      ...
      sessionStorage.setItem('totalApplicants', data.data.length);
      
      // Update the counter in the dashboard if it exists on this page
      updateTotalApplicantsCounter(data.data.length);
...
async function handleSearch(e) {
  ...
  try {
    const response = await fetch(`/api/admin/applicants?search=${encodeURIComponent(searchTerm)}`, {
      credentials: 'include'
    });
