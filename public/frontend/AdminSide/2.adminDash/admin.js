...
function updateDashboardStats() {
  ...
  const newApplicantsCount = applicants.filter(a => 
    a.status && a.status.toLowerCase() === 'pending').length;
  const withoutAssessorCount = applicants.filter(a => 
    !a.assessorId || a.assessorId === '').length;
  const rejectedCount = applicants.filter(a => 
    a.status && a.status.toLowerCase() === 'rejected').length;
...
function renderApplicantsTable(applicantsToRender) {
  ...
  applicantsToRender.forEach(applicant => {
    ...
    // Format date
    const appDate = new Date(applicant.applicationDate || applicant.dateAssigned || new Date());
    ...
    row.innerHTML = `
      ...
      <td class="action-buttons">
        <a href="/frontend/AdminSide/Evaluation/evaluation.html?id=${applicant._id}" class="action-btn view-btn">
